...
does real tool use. Fine-tuning (phase0_sft / phase1_dpo) then improves
tool-call accuracy, and the eval harnesses measure it.
"""
...
class Agent:
    def __init__(self, model_name=DEFAULT_MODEL, device=None, adapter=None,
                 merge_adapter=None, dtype=torch.bfloat16, load_4bit=False):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.tokenizer = load_tokenizer(model_name)
        self.model = load_with_adapters(
            model_name, adapter=adapter, merge_adapter=merge_adapter,
            dtype=dtype, device_map=self.device, load_4bit=load_4bit)
        if load_4bit:                                  # device_map='auto' -> feed inputs to the
            self.device = self.model.device            # embedding layer's actual device
        self.model.eval()
